...
samples=$(grep $SAMPLE ${WORKDIR}/${SAMPLELIST}Full | cut -f1 | awk -v workdir=${WORKDIR} '{printf "I="workdir"/"$0".sorted.bam "}')
...
time(java -jar $EBROOTPICARD/picard.jar MarkDuplicates $samples O=${WORKDIR}/${SAMPLE}.nodup.bam CREATE_INDEX=true TMP_DIR=${WORKDIR}/${SAMPLE}_Temp METRICS_FILE=${WORKDIR}/${SAMPLE}.MarkDuplicatesMetrics.txt VALIDATION_STRINGENCY=LENIENT REMOVE_DUPLICATES=TRUE)
# modified 21/01/2019
echo "FINISHED"
