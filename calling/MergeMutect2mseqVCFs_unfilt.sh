...
echo "I="${WORKDIR}/Mutect2_mseq.$i.${PATIENT}.vcf" "; 
done; 
echo "I="${WORKDIR}/Mutect2_mseq.X.${PATIENT}.vcf " "; 
echo "I="${WORKDIR}/Mutect2_mseq.Y.${PATIENT}.vcf " "; 
echo "I="${WORKDIR}/Mutect2_mseq.MT.${PATIENT}.vcf " ";)

java -jar $EBROOTPICARD/picard.jar MergeVcfs \
	$INPUTS \
	D=${RESDIR}/${REF}.dict \
	O=${WORKDIR}/${PATIENT}.unfilt.vcf \
